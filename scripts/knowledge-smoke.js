...
require('./fixtures/knowledge-index').writeKnowledgeIndex(home,[{id:'knowledge-fixture',file:path.toNamespacedPath(path.join(home,'sessions','fixture.jsonl')),title:'额度同步验证'}],{name:'Codex Auth Manager',root:'/sample/project'});
let calls=0,hold=false;
require(path.join(sourceRoot,'knowledge/cli')).executeDistillation=async({prompt,model,reasoningEffort,signal})=>{calls++;if(calls===1){assert.equal(model,'fixture-deep');assert.equal(reasoningEffort,'xhigh');}assert.ok(!prompt.includes('这条是未选择的合成消息，不参与蒸馏。'));if(hold)await new Promise((resolve,reject)=>signal.addEventListener('abort',()=>reject(Error('已取消蒸馏。')),{once:true}));return '## 已决定事项\n\n保留同周期最后有效的周额度。[1:1]\n\n## 验证方法\n\n同时检查主窗口、悬浮窗与日志轮换。[1:2]\n\n## 适用边界\n\n新周期正常重置允许显示 100%。';};
...
(async()=>{
 ...
 await click('[data-page="distill"]');await click('[data-kact="sessions"]');await until(()=>evaluate('!!document.querySelector(".k-session")'),'active row');
