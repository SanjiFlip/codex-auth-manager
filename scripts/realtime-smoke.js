...
(async()=>{
  ...
  const before=calls;
  await Promise.all([evaluate(main,'window.codexAuth.refreshCurrentQuota()'),evaluate(meter,'window.codexAuth.refreshCurrentQuota()')]);
  assert.equal(calls,before,'Two windows must not bypass official cooldown');
  assert.equal(fs.readFileSync(path.join(home,'auth.json'),'utf8'),auth);
  console.log('REALTIME PASS: log changes, cross-window quota, cooldown, current auth preserved');app.exit(0);
