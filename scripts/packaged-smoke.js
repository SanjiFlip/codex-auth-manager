...
(async()=>{
  try{
    ...
    assert.equal(state.version,'0.3.7');assert.equal(state.accounts[0].planType,'prolite');
    ...
    console.log('PACKAGED PASS: EXE 0.3.7 startup, isolated vault, real IPC/DPAPI, Pro 5x, shared theme, native meter, pin, missing quota, unpacked Windows helper, current auth unchanged.');
