...
(async()=>{
  ...
  await run(`document.querySelector('[data-action="add"]').click()`);
  await run(`document.querySelector('[data-action="login"]').click()`);
  await until(()=>run(`window.codexAuth.getLoginState().then(s=>s.phase==='waiting')`));
  const normalChild=child;
  child.stderr.write('Error logging in: callback bind failed (os error 10013)\n');child.emit('close',1);
  await until(()=>child!==normalChild);
  child.stderr.write('https://auth.openai.com/codex/device\nABCD-12345\n');
  await until(()=>run(`document.querySelector('#login-device-code').textContent==='ABCD-12345'`));
  assert.equal(await run(`document.querySelector('#login-device').hidden`),false);
  await until(()=>opened.length===3);
  assert.equal(opened[2],'https://auth.openai.com/codex/device');
  await run(`window.codexAuth.cancelLogin()`);
  assert.equal(await run(`document.querySelector('#login-device').hidden`),true);
  assert.deepEqual(fs.readdirSync(path.join(root,'vault','pending-logins')),[]);
  console.log('BROWSER SMOKE PASS: login IPC, browser retry, blocked-port fallback, visible device code, cancellation.');
