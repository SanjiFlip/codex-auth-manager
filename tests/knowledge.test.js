...
test('workbench sends only selected messages, redacts secrets and persists a draft with provenance',async()=>{
 ...
 const wb=createWorkbench({library,store,resolveModel:async()=>({model:'test-model',reasoningEffort:'high'}),withAccount:fn=>fn(),execute:async x=>{prompt=x.prompt;assert.equal(x.model,'test-model');assert.equal(x.reasoningEffort,'high');return require('../scripts/fixtures/distillation-model')(x)}});
...
test('CLI uses hidden native process, sends material over stdin and collects UTF-8 result',async()=>{
 let received='',cwd;
 const output=await executeDistillation({home:'test-home',prompt:'PRIVATE-INPUT',outputSchema:{type:'object'},model:'test-model',reasoningEffort:'xhigh',resolve:async()=>({command:'fixture',args:[]}),spawnProcess:fixture((c,args,options)=>{
  cwd=options.cwd;const schemaPath=args[args.indexOf('--output-schema')+1];assert.deepEqual(JSON.parse(require('node:fs').readFileSync(schemaPath,'utf8')),{type:'object'});assert.equal(args[args.indexOf('--model')+1],'test-model');assert.ok(args.includes('model_reasoning_effort="xhigh"'));assert.equal(options.windowsHide,true);assert.equal(options.shell,false);assert.equal(options.env.CODEX_HOME,'test-home');assert.ok(args.includes('--ignore-user-config'));assert.ok(args.includes('--ephemeral'));assert.ok(args.includes('features.shell_tool=false'));assert.ok(!args.includes('PRIVATE-INPUT'));c.stdin.on('data',x=>received+=x);
