...
现有的速API、简心API、MyMemory 均无需 Key，可按需勾选。翻译仍默认关闭，已有开关与服务选择保持不变。重新打开详情时先显示原文，点击「翻译正文」才恢复完整译文缓存或发送请求。所选文本会发送给勾选的第三方，不改写技能文件；免费额度和网络连通性没有保证。[翻译说明与服务限制](docs/SKILLS.md#可选中文翻译)。
