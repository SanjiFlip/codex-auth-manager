...
function createWorkbench({library,store,execute,withAccount,resolveModel,report=()=>{}}){
  ...
  async function prepare(request){
    ...
    for(const selected of request.selection){const meta=items.find(x=>x.id===selected.id);if(!meta||!Array.isArray(selected.messages)||!selected.messages.length||selected.messages.length>500)throw Error('所选素材已变化，请重新选择。');
  ...
  return {
    ...
    start(request){if(active)throw Error('已有蒸馏任务正在运行。');const job={abort:new AbortController()};active=job;publish({phase:'preparing',startedAt:new Date().toISOString()});
      job.done=(async()=>{try{const prepared=await prepare(request);if(job.abort.signal.aborted)throw Error('已取消蒸馏。');const body=await withAccount(async()=>{
          if(job.abort.signal.aborted)throw Error('已取消蒸馏。');const modelSelection=await resolveModel(request);if(job.abort.signal.aborted)throw Error('已取消蒸馏。');publish({...state,phase:'running',...modelSelection});return execute({prompt:prepared.prompt,...modelSelection,signal:job.abort.signal});
        });
