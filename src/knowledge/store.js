...
function createKnowledgeStore({root,encrypt,decrypt}){
  ...
  return {
    async list(){await queue;return (await read()).items.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt))},
    save(input,{generated=false}={}){const fields=content(input);return mutate(data=>{
      ...
      const now=new Date().toISOString();const item={...fields,id:old?.id||randomUUID(),revision:(old?.revision||0)+1,createdAt:old?.createdAt||now,updatedAt:now,status:input.status==='draft'?'draft':'saved',origin:old?.origin||(generated?'distill':'manual'),sources:old?.sources||(generated?input.sources:[])||[]};
