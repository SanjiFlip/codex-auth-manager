...
async function act(action,id) {
  ...
  if(action==='switch-confirm'||action==='switch-force-confirm')return run(async()=>{
    ...
    try{state=await api.switchAccount(id,{restartCodex:true,forceClose:action==='switch-force-confirm'});render();log('已替换凭据并重启 Codex，身份待核对');toast(isDemo?'演示切换完成，未操作真实 Codex':'Codex 已重启，请核对账号身份')}
      ...
      modal('Codex 仍在托盘运行','<p>关闭窗口后，Codex 仍可能驻留在右下角托盘。请右键托盘图标并选择退出，然后重试。</p><div class="modal-info">凭据尚未改写。若无法从托盘退出，可在保存任务后选择结束后台。</div>',button('取消','close')+button('结束后台并切换','switch-force-prompt','','',`data-id="${escape(id)}"`)+button('已从托盘退出，重试','switch-confirm','primary','',`data-id="${escape(id)}"`));
    }
  });
