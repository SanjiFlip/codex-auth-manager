...
function renderUsage() {
  ...
  $('#content').innerHTML=heading('用量概览','从每日趋势到模型分布，了解你的本机 Codex 使用情况。',button('刷新本机统计','statistics-refresh','primary','refresh'))+
    ...
    '<div class="panel"><h2>每日明细</h2>'+data.daily.map(d=>'<div class="setting-row"><strong>'+d.day+'</strong><span class="help-text">'+number(d.tokens)+' tokens · '+d.sessions+' 个会话</span></div>').join('')+'<p class="help-text">已扫描 '+number(data.scannedFiles)+' 个文件；读取失败 '+number(data.failedFiles)+' 个。这里只反映本机可读取的记录，不代表其他设备或订阅总用量。</p></div>';
}
...
function loginView(status) {
  ...
  el.textContent=status.browserError||(status.phase==='waiting'&&!status.url&&!isDemo?'正在准备官方授权链接…':messages[status.phase]||'');
...
async function act(action,id) {
  ...
  if(action==='switch-confirm')return run(async()=>{$('#modal').close();log('开始切换账号');state=await api.switchAccount(id,{restartCodex:true});render();log('已替换凭据并重启 Codex，身份待核对');toast(isDemo?'演示切换完成，未操作真实 Codex':'Codex 已重启，请核对账号身份')});
