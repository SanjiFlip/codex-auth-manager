...
核对日期：2026-09-12。上游固定提交见 REFERENCE_REVIEW.md。以下区分源码实现与真实账号验收。

| 能力 | 参考来源 | 本版落地 |
| --- | --- | --- |
| 本机凭据加密与账号库 | GboyCode：DPAPI、快照、索引恢复 | 继承加密与恢复基础，改用独立目录；真实 DPAPI + 合成账号已验证 |
| 添加新账号 | Mintimate：Device Code 授权 | 使用官方 CLI 浏览器 OAuth，独立临时目录；取消、保存与清理已测试，真人授权待验收 |
...
- 切换状态与回滚：src/switch-lifecycle.js；Windows 进程适配：src/windows-codex.js / .ps1。
