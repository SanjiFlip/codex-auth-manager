...
已发布版本：v0.4.1；v0.5.0 新功能见下文；核对日期：2026-09-21。上游固定提交见 REFERENCE_REVIEW.md。以下区分源码实现与真实账号验收。
...
## v0.5.0 开发中：独立实现的蒸馏与记忆

- 蒸馏工作台：Codex 本地 JSONL 会话检索、跨会话消息选择、五类产物、模型与要求配置、显式确认、取消与草稿历史。
- 记忆库：本地加密存储，按项目 / 类型 / 状态检索，编辑、删除、来源追溯与 Markdown 导出。
- 使用本机 Codex CLI 当前账号，不接入 aitracker 的模型服务、不复制其代码；当前不支持自动安装 Skill、自动注入 Codex 记忆或多工具会话扫描。
- Windows 隔离集成与合成文本真实 CLI 调用已验证；Mac 新功能待本版专项验证。使用限制见 [KNOWLEDGE](KNOWLEDGE.md)。
